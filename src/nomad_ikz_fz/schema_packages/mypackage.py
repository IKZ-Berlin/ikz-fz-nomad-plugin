...
class Assembly(PlotSection, ArchiveSection):
    """
    Class autogenerated from yaml schema.
    """
    m_def = Section(a_eln=dict(overview=True))
...
class AssemblyStep(FzGrowthStep, ArchiveSection):
    m_def = Section(a_eln=dict(overview=True))
    reference_fzassembly = Quantity(
        type=FzAssembly,
        a_eln={'component': 'ReferenceEditQuantity'},
    ...
    def normalize(self, archive: 'EntryArchive', logger: 'BoundLogger') -> None:
        ...
        """
        super().normalize(archive, logger)
        # if self.reference_fzassembly: 
        #     self.assembly = self.reference_fzassembly.assembly
        # if self.save_assembly == True and self.assembly_name and self.assembly:
        #     new_assembly = FzAssembly()
        #     new_assembly.assembly = self.assembly
        #     filename=self.assembly_name+".archive.json" 
        #     create_archive(new_assembly, archive, filename)
...
# class Instruments(InstrumentReference,  ArchiveSection):
#     m_def = Section()
#     def normalize(self, archive: 'EntryArchive', logger: 'BoundLogger') -> None:
#         """
#         Args:
...
class FzFurnace(FzInstrument, EntryData, ArchiveSection):
    m_def = Section()
class FzInstrumentReferenz(InstrumentReference):
    m_def = Section(a_eln=dict(overview=True))
    reference = Quantity(
        type=FzAssembly, #FzInstrument,
...
class FzGrowthProcess(Process, EntryData, ArchiveSection):
    ...
    instruments = SubSection(
        section_def=FzInstrumentReferenz,
        repeats=True,
    )
    fz_furnace= Quantity(
    ...
    def normalize(self, archive: 'EntryArchive', logger: 'BoundLogger') -> None:
        ...
        #     if step.m_def == "nomad_ikz_fz.schema_packages.mypackage.AssemblyStep":
        #         if step.reference_fzassembly: 
        ...
        #instruments=[]
        # assemblyref=FzInstrumentReferenz()
        for step in self.steps:
            if isinstance(step, AssemblyStep): #. == "nomad_ikz_fz.schema_packages.mypackage.AssemblyStep":
                if step.reference_fzassembly and not step.save_assembly: 
                    step.assembly = step.reference_fzassembly.assembly
                    #instruments.append(step.reference_fzassembly.m_proxy_value) # figure ou here
                    
                # if step.reference_fzassembly and not step.save_assembly:# and step.assembly_name == None:
                #     #assemblyref=step.reference_fzassembly
                #     #self.test=assemblyref
                #     instruments.append(step.reference_fzassembly) #assemblyref)
                elif step.save_assembly and step.assembly_name is not None:
                    new_assembly = FzAssembly()
                    new_assembly.assembly = step.assembly
                    filename=step.assembly_name+".archive.json" 
                    reference = create_archive(new_assembly, archive, filename)
                    # reference=f'../uploads/{archive.m_context.upload_id}/archive/{hash(archive.m_context.upload_id, filename)}#data',
                    #instruments.append(reference)
        #self.instruments=instruments
        super().normalize(archive, logger)
        # furnace=FzFurnace()
        # if self.fz_furnace == "Fz 30":
        #     furnace.lab_id="IKZ_Fz_30"
        # elif self.fz_furnace == "Fz 1505":
        #     furnace.lab_id="IKZ_Fz_1505"
        # elif self.fz_furnace == "Fz 1520":
        #     furnace.lab_id="IKZ_Fz_1520"
        # self.instruments=[furnace]
        # if self.steps is None:
        #     self.steps[0] = Assembly()
